...
class ExactInference(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        for row in range(self.belief.getNumRows()):
            for column in range(self.belief.getNumCols()):
                d = math.sqrt((agentX - util.colToX(column))**2 + (agentY - util.rowToY(row))**2)
                prob = util.pdf(observedDist, Const.SONAR_STD, d)
                self.belief.setProb(row, column, prob*self.belief.getProb(row, column))

        if self.belief.getSum() != 1: self.belief.normalize()
        # END_YOUR_CODE
    ...
    def elapseTime(self):
        ...
        # Create new belief
        newBelief = util.Belief(self.belief.getNumRows(), self.belief.getNumCols(), value=0)

        # Loop through all oldtile, newtile transprob values
        for oldTile, newTile in self.transProb.keys():
            # If the probability is equal to zero, continue to next loop, will not add probability
            if self.transProb[(oldTile, newTile)] == 0:
                continue
            # Add in new probability given old and new tile values, multiplied by the probability of being on oldtile
            prob_i = self.belief.getProb(oldTile[0], oldTile[1])
            newBelief.addProb(newTile[0], newTile[1], prob_i*self.transProb[(oldTile, newTile)])

        # Set new belief at new time step equal to the current belief for the game
        self.belief = newBelief

        # Normalize new belief if sum isn't already equal to 1
        if self.belief.getSum() != 1: self.belief.normalize()
        # END_YOUR_CODE
...
class ParticleFilter(object):
    ...
    def observe(self, agentX, agentY, observedDist):
        ...
        def distance(t):
            """
            Find the distance between specific tile and the agent
            """
            return math.sqrt((agentX - util.colToX(t[1])) ** 2 + (agentY - util.rowToY(t[0])) ** 2)

        def weight(t):
            """
            Return the weight based off the pdf for a certain distance between agent and tile,
            feed this information directly into weights dict
            """
            return util.pdf(observedDist, Const.SONAR_STD, distance(t))*self.particles[t]

        # Calculate weights particles sampled based on the pdf distribution
        for tile in self.particles:
            weights[tile] = weight(tile)

        # Re-sampling of particles using weights calculated from above
        resampledParticles = collections.defaultdict(int)
        for i in range(self.NUM_PARTICLES):
            resampledParticles[util.weightedRandomChoice(weights)] += 1

        # Update so that resampled particles are new particles in problem
    ...
    def elapseTime(self):
        # BEGIN_YOUR_CODE (our solution is 6 lines of code, but don't worry if you deviate from this)'
        # Create new particles dict to store updated particles from position switch
        updatedParticles = collections.defaultdict(int)

        # For each particle on a particular tile, more using random chance to same or adjacent tile
        for old_tile in self.particles:
            for _ in range(self.particles[old_tile]):
                updatedParticles[util.weightedRandomChoice(self.transProbDict[old_tile])] += 1

        # Change the particles to the updated ones
