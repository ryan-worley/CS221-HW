...
def create_chain_csp(n):
    ...
    for i in range(len(variables)-1): csp.add_binary_factor(variables[i], variables[i+1], xor)
    # END_YOUR_CODE
    return csp
...
def create_nqueens_csp(n = 8):
    ...
    for x in range(n):
        small_domain = []
        for y in range(n):
            small_domain.append((x, y))
        domain.append(small_domain)

    # Function for seeing if two pieces are in diagonal
    def isnotdiagonal(d1, d2):
        x1,y1 = d1
        x2,y2 = d2
    ...
    # Function for seeing if two pieces are in row
    def isnotrow(d1, d2):
        if d1[1] != d2[1]:
            return 1
        return 0

    # Add all variables with proper domain
    for i in range(len(variables)):
        csp.add_variable(variables[i], domain[i])

    # Add the proper binary constraints
    for i in range(len(variables)):
        for j in range(len(variables)):
            if i != j:
                csp.add_binary_factor(variables[i], variables[j], isnotdiagonal)
                csp.add_binary_factor(variables[i], variables[j], isnotrow)
    return csp
            ...
            for var in self.csp.variables:
                if var not in assignment:
                    ...
                    for dom in self.domains[var]:
                        w = self.get_delta_weight(assignment, var, dom)
                        if w > 0: counter += 1
                    counters.append(counter)
            best_index = counters.index(min(counters))
            return vars[best_index]
            # END_YOUR_CODE
    ...
    def arc_consistency_check(self, var):
        ...
        while len(queue) != 0:
            ...
            for var2 in self.csp.get_neighbor_vars(var1):
                flag = False
                neighbor_domains_unedited = copy.copy(self.domains[var2])
                for val2 in neighbor_domains_unedited:
                    remove = True
                    for val1 in self.domains[var1]:
                        weight = self.csp.binaryFactors[var1][var2][val1][val2]
                        # No failure of binary domains of at least one option, satisfy constraint
                        if weight > 0:
                            # If one instance has positive weight, we good, move on
                            remove = False
                            break
                    if remove:
...
def get_sum_variable(csp, name, variables, maxSum):
    ...
    for i, X_i in enumerate(variables):
        ...
        # Find all combination of domain
        combinations = [(v1, v2) for v1 in range(maxSum + 1) for v2 in range(maxSum + 1)]
        csp.add_variable(A_i, combinations)

        def factorAdd(x, a):
            return a[1] == a[0] + x

        csp.add_binary_factor(X_i, A_i, factorAdd)

        def factorConsistent(a1, a2):
            return a1[1] == a2[0]

        def isZero(a1):
            return a1[0] == 0

        # first iteration thru:
        if i == 0:
            csp.add_unary_factor(A_i, isZero)
        else:
            csp.add_binary_factor(('sum', name, i-1), A_i, factorConsistent)

    def factorConsistentResult(a1, result):
        return a1[1] == result

    csp.add_binary_factor(A_i, result, factorConsistentResult)
    return result
    # END_YOUR_CODE
...
class SchedulingCSPConstructor():
    ...
    def add_quarter_constraints(self, csp):
        ...
        for quarter in self.profile.quarters:
            for request in self.profile.requests:
                def quarter_constraint(course_id):
                    if course_id is None: return True
                    if quarter in request.quarters: return True
                    if len(request.quarters) == 0: return True
                    else: return False
                csp.add_unary_factor((request, quarter), quarter_constraint)
        ...
        for quarter in self.profile.quarters:
            ...
            for request in self.profile.requests:
                if len(request.cids) == 0:
                    continue  # Go to next loop, don't continue, will create error in course_id loop
                for course_id in request.cids: # For all course ids in a certain request

                    # Function used for binary constraint
                    def constraint_function(units, cid_constraint):
                        if cid_constraint == course_id:
                            return units != 0
                        else:
                            return units == 0

                    # Set new variable as course_id
                    variable = (course_id, quarter)
                    variables.append(variable)

                    # set domain of units for each course, 0 being the not take and min and max units otherwise
                    no_take_range = [0]
                    take_range = range(self.bulletin.courses[course_id].minUnits, self.bulletin.courses[course_id].maxUnits+1)

                    # Add variable for each course_id in requests, quarter
                    csp.add_variable(variable, no_take_range + take_range)

                    # Add binary constraints for the problem, (req, quarter) already added as variable
                    constraint_variable = (request, quarter)  # Already added in
                    csp.add_binary_factor(variable, constraint_variable, constraint_function)

            # Add in sumVar constraint
            def unitConstraint(n):
                if self.profile.minUnits <= n<= self.profile.maxUnits:
                    return True
                else: return False

            # Add the sumVariable Constraint to the problem, similiar to how is done in grader.py
            sumVariable = get_sum_variable(csp, 'UnitCap' + str(quarter), variables, self.profile.maxUnits)
            csp.add_unary_factor(sumVariable, unitConstraint)
        # END_YOUR_CODE
