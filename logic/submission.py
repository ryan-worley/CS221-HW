...
def formula1a():
    ...
    return Implies(And(Atom('California'),
                       Atom('Summer')), Not(Atom('Rain')))
    # END_YOUR_CODE
...
def formula1b():
    ...
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    return Equiv(Or(Atom('Rain'), Atom('Sprinklers')), Atom('Wet'))
    # END_YOUR_CODE
...
def formula1c():
    ...
    # BEGIN_YOUR_CODE (our solution is 1 line of code, but don't worry if you deviate from this)
    # Not(one) and two or not(two) and one

    return Or(And(Atom('Day'), Not(Atom('Night'))),
              And(Atom('Night'), Not(Atom('Day'))))
    # END_YOUR_CODE
...
def formula2b():
    ...
    return Exists('$x',
                  Forall('$y',
                         AndList([Person('$x'), Not(Child('$x', '$y'))])))
    # END_YOUR_CODE
...
def formula2c():
    ...
    return Forall('$y', Forall('$x', Equiv(And(Female('$x'), Child('$y', '$x')), Daughter('$y', '$x'))))
    # END_YOUR_CODE
...
def formula2d():
    ...
    return Forall('$x', Forall('$z', Equiv(Or(Exists('$y',
                AndList([
                    Parent('$x', '$y'),
                    Parent('$y', '$z'),
                    Female('$z')])),
...
def liar():
    def TruthConstraint(truth, lie, liee, lieeee):
        return AndList([TellTruth(truth), Not(TellTruth(lie)), Not(TellTruth(liee)), Not(TellTruth(lieeee))])
    def CrashConstraint(crasher, Lana, Lanaaaa, LANAAAAA):
        return AndList([CrashedServer(crasher), Not(CrashedServer(Lana)),
                        Not(CrashedServer(Lanaaaa)), Not(CrashedServer(LANAAAAA))])
    ...
    # 4: Use constraints to make sure one of 4 unique situations is true. Okay not to use xor here because no situations
    # Can ever be true at once.
    formulas.append(OrList([TruthConstraint(john, susan, nicole, mark),
                            TruthConstraint(susan, john, nicole, mark),
                            TruthConstraint(nicole, susan, john, mark),
                            TruthConstraint(mark, susan, nicole, john)]))
    # 5 Use constraints to make sure one of 4 unique situations is true. Okay not to use xor here because no situations
    # Can ever be true at once.
    formulas.append(OrList([CrashConstraint(john, susan, nicole, mark),
                            CrashConstraint(susan, john, nicole, mark),
                            CrashConstraint(nicole, susan, john, mark),
                            CrashConstraint(mark, susan, nicole, john)]))
...
def ints():
    ...
    formulas.append(Forall('$x', Exists('$y', AndList([Successor('$x', '$y'), Not(Equals('$x', '$y')),
                    Forall('$z', Implies(AndList([Successor('$x', '$y'), Successor('$x', '$z')]),
                        AndList([Equals('$y', '$z'), Not(Equals('$x', '$y'))])))]))))
    # Law 1
    ...
    # Law 5
    formulas.append(Forall('$x', Forall('$y', Forall('$z', Implies(And(Larger('$z', '$y'), Larger('$y', '$x')), Larger('$z', '$x'))))))
    # END_YOUR_CODE
