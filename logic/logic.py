...
def applySubst(form, subst):
    ...
    if form.isa(Atom): return apply(Atom, [form.name] + [applySubst(arg, subst) for arg in form.args])
...
def substituteFreeVars(form, var, obj):
    def recurse(form, boundVars):
        if form.isa(Variable):
            ...
            return apply(Atom, [form.name] + [recurse(arg, boundVars) for arg in form.args])
...
class ToCNFRule(UnaryRule):
    ...
    def applyRule(self, form):
        ...
        def updateSubst(subst, var):
            ...
            return dict(subst.items() + [(var, newVar)])
        def standardizeVariables(form, subst):
            if form.isa(Variable):
                if form not in subst: raise Exception("Free variable found: %s" % form)
                return subst[form]
            if form.isa(Constant): return form
            if form.isa(Atom): return apply(Atom, [form.name] + [standardizeVariables(arg, subst) for arg in form.args])
        ...
        def skolemize(form, subst, scope): 
            ...
            if form.isa(Constant): return form
            if form.isa(Atom): return apply(Atom, [form.name] + [skolemize(arg, subst, scope) for arg in form.args])
            ...
            if form.isa(Exists):
                ...
                if len(scope) == 0:
                    ...
                    skolem = apply(Atom, ['Skolem' + form.var.name, form.var] + scope)
...
def propositionalize(forms, objects=None):
    ...
    def convert(form, subst):
        ...
        if form.isa(Constant): return form
        if form.isa(Atom):
            return apply(Atom, [form.name] + [convert(arg, subst) for arg in form.args])
        ...
        if form.isa(Exists):
            return OrList([convert(form.body, dict(subst.items() + [(form.var, obj)])) for obj in objects])
        if form.isa(Forall):
            return AndList([convert(form.body, dict(subst.items() + [(form.var, obj)])) for obj in objects])
        raise Exception("Unhandled: %s" % form)
...
def universalInterpret(form):
    ...
    if form.isa(Atom):
        ...
        return apply(Atom, [form.name] + [universalInterpret(arg) for arg in form.args])
    if form.isa(Not):
...
def showKBResponse(response, verbose=1):
    if isinstance(response, KBResponse):
        ...
        items = [(obj, r.status) for ((var, obj), r) in response.items()]
...
class KnowledgeBase:
    ...
    def query(self, form, modify):
        ...
        if len(freeVars) > 0:
            ...
            var = freeVars[0]
            allForms = AndList([deriv.form for deriv in self.derivations.values()])
    ...
    def addDerivation(self, deriv):
        ...
        if oldDeriv == None and deriv.cost <= maxCost:
            ...
            if self.modelChecking:
                allForms = [deriv.form for deriv in self.derivations.values()]
            ...
            for key2, deriv2 in self.derivations.items():
